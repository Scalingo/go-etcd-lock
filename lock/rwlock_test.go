...
func TestRWLockAcquireWrite(t *testing.T) {
	...
	t.Run("wait acquire read waits behind a pending writer", func(t *testing.T) {
		readLock, err := locker.AcquireRead("/rw-pending-writer", 1)
		require.NoError(t, err)
		...
		go func() {
			lock, err := locker.WaitAcquireWrite("/rw-pending-writer", 1)
			writeErr <- err
			writeReady <- lock
			writeAt <- time.Now()
		}()
		...
		t1 := time.Now()
		go func() {
			lock, err := locker.WaitAcquireRead("/rw-pending-writer", 1)
			readErr <- err
			readReady <- lock
			readAt <- time.Now()
		}()
